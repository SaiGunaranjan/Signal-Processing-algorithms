...
theoreticalSLdB_rect = -3.92
theoreticalSLdB_hann = -1.42
...
plt.plot(spectruma2_rect,alpha=0.5, lw=4, label='signal on bin {}'.format(binNum+0.5))
plt.axhline(theoreticalSLdB_rect,color='black',linestyle='dashed', label='Theoretical value = {} dB'.format(theoreticalSLdB_rect))
plt.grid(True)
...
plt.plot(spectruma2_hann,alpha=0.5, lw=4, label='signal on bin {}'.format(binNum+0.5))
plt.axhline(theoreticalSLdB_hann,color='black',linestyle='dashed', label='Theoretical value = {} dB'.format(theoreticalSLdB_hann))
plt.grid(True)
plt.xlabel('bin Number')
plt.ylim([-10,3])
plt.xlim([binNum-3, binNum+3])
...
plt.plot(spectruma2_apod,alpha=0.5, lw=4, label='signal on bin {}'.format(binNum+0.5))
plt.grid(True)
plt.xlabel('bin Number')
plt.ylim([-10,3])
plt.xlim([binNum-3, binNum+3])
