...
""" Define bi-polar frequency indices of the signals/sinusoids present in the signal"""
...
filterResponseSuperSet = np.conj(vandermondeMatrixSuperSet) # This has to have a conjugation effect since pseudo inverse has an inherent conjugation
...
plt.plot(freAxis_bipolar, 20*np.log10(np.abs(vandermondeMatrixInv_fft[1,:])), color='k', label='Inv Vand Mat 2nd row spectrum')
...
# plt.plot(freAxis_bipolar, 20*np.log10(np.abs(vandermondeMatrixInv_fft[1,:])/np.amax(np.abs(vandermondeMatrixInv_fft[1,:]))), color='k', label='Inv Vand Mat 1st row spectrum')
plt.plot(freAxis_bipolar, np.unwrap(np.angle(vandermondeMatrixInv_fft[1,:])), color='k', label='Inv Vand Mat 2nd row spectrum')
plt.axvline(-digFreq[1] , color='b', label='signal')
plt.axvline(-digFreq[0] ,color='r', label='interferer')
# plt.plot(freAxis_bipolar, 20*np.log10(np.abs(sig2_tf_fft)/(np.amax(np.abs(sig2_tf_fft)))), label='synthesized response for sig2')
