...
signal_spectrum = np.fft.fft(range_signal_with_noise)[0:num_signal_samples//2] # compute the fft of the noisy signal
...
eps_noise = noise_sigma*np.exp(1j*np.random.uniform(low=-np.pi,high=np.pi,size=num_zeros)) # add some small noise to the zero entries (just to compare the reconstructed signal spectrum with the true signal spectrum)
recon_signal_spectrum[np.abs(recon_signal_spectrum)==0] = eps_noise
plt.figure(1)
plt.plot(np.arange(0,np.pi,2*np.pi/num_signal_samples), 20*np.log10(np.abs(signal_spectrum)),label='True Signal Spectrum')
plt.vlines(sig_freq,-40,120, label = 'Ground truth')
plt.plot(np.arange(0,np.pi,2*np.pi/num_signal_samples), 20*np.log10(np.abs(recon_signal_spectrum)),label='Reconstructed Signal Spectrum')
plt.grid(True)
