...
for snr in SNR:
    ...
    for iteration in np.arange(num_montecarlo):
        ...
        pseudo_spectrum, digital_freq_grid_music = spec_est.music_backward(noisy_signal, num_sources, corr_mat_model_order)
        est_dig_freq_music = float(digital_freq_grid_music[np.argsort(pseudo_spectrum)[-num_sources::]])
        est_dig_freq_esprit = float(-1*spec_est.esprit_backward(noisy_signal, num_sources, corr_mat_model_order))
        psd_b, digital_freq_grid_capon = spec_est.capon_backward(noisy_signal, corr_mat_model_order)
        est_dig_freq_capon = digital_freq_grid_capon[np.argmax(psd_b)]
        magnitude_spectrum, digital_freq_grid_apes = spec_est.apes(noisy_signal, corr_mat_model_order)
        est_dig_freq_apes = digital_freq_grid_apes[np.argmax(magnitude_spectrum)]
...
tend = time.time()
print("Time elapsed = {} secs".format(tend-tstart))        
plt.figure(1)
