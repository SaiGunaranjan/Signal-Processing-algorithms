...
mu, G = sai_pomp.mutual_coherence(overall_mat)
phi_max = 1/2*np.arccos(mu)*np.ones(overall_mat.shape[1])
pomp_num_iters = 40
pomp_tol = 1e-60
residual_threshold = 1e-4
xhat, xhat_all, x_nonzInd = sai_pomp.POMP_sai(overall_mat_normalized, sub_sampl_signal, pomp_num_iters, phi_max, pomp_tol, residual_threshold)
